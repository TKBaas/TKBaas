...
public class SaleOrderPCAction {
	...
	public String add(HttpServletRequest request, Model model){
		...
		if(user == null)return "redirect:/pc/login";
	...
	public String submit(HttpServletRequest request,
			            @ModelAttribute("OrderForm") OrderSubmitForm orderSubmitForm, Model model){
		...
		model.addAttribute("OrderForm", orderSubmitForm);
		User user = (User)request.getSession().getAttribute("user");
		if(user == null)return "redirect:/user/pc/login";
	...
	public String submitOrderSuccess(HttpServletRequest request, @ModelAttribute("OrderForm") OrderSubmitForm orderSubmitForm,Model model) {
		...
		for(int i=0; i<proIds.length;i++) {
			Product product = productService.getProduct(proIds[i]);
			nameList.add(product.getName());
		}
		
		model.addAttribute("nameList", nameList);
		
		return "/user/submitOrderSuccess";
	}*/
	...
	public void getSearchUserOrder(HttpServletRequest request,
		...
		try{
			...
			for(SaleOrder saleOrder : list){
				JSONObject orderJson = new JSONObject();
				orderJson.put("boughtDate", saleOrder.getBoughtDate().toString());
				orderJson.put("id", saleOrder.getId());
				orderJson.put("money", saleOrder.getMoney());
				orderJson.put("sellerId", saleOrder.getSellerId());
				orderJson.put("state", saleOrder.getState());
